...
require_once 'dbconfig.php';
	...
	{
		$id = $_GET['edit_id'];		
		$stmt_edit = $DB_con->prepare('SELECT bigdata, smalldata, userPic FROM bastrocar WHERE userID =:uid');
	...
	{
		header("Location: adminBastrobank.php");
	}
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		else
		{
			...
			$folder = $edit_row['userPic']; // old image from database
		}	
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE bastrocar 
									     SET bigdata=:bigdata, 
										     smalldata=:smalldata, 
										     userPic=:upic 
								       WHERE userID=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='adminBastrobank.php';
				</script>
                <?php
			}
			else{
...
<form method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    <tr>
    	<td><label class="control-label">Carousel Img.</label></td>
        <td>
