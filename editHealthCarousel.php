...
require_once 'dbconfig.php';
	...
	{
		$id = $_GET['edit_id'];		
		$stmt_edit = $DB_con->prepare('SELECT bigdata, smalldata, picsource FROM healthcar WHERE id =:uid');
	...
	{
		header("Location: adminhealthcare.php");
	}
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE healthcar 
									     SET bigdata=:bigdata, 
										     smalldata=:smalldata, 
										     picsource=:upic 
								       WHERE id=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='adminhealthcare.php';
				</script>
                <?php
			}
			else{
