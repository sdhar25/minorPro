...
require_once 'dbconfig.php';
	...
	{
		$id = $_GET['edit_id'];		
		$stmt_edit = $DB_con->prepare('SELECT bigdata, smalldata, picsource FROM medteamcar WHERE id =:uid');
	...
	{
		header("Location: adminMedicalTeam.php");
	}
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE medteamcar 
									     SET bigdata=:bigdata, 
										     smalldata=:smalldata, 
										     picsource=:upic 
								       WHERE id=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='adminMedicalTeam.php';
				</script>
                <?php
			}
			else{
