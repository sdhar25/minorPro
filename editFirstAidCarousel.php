...
require_once 'dbconfig.php';
	...
	{
		$id = $_GET['edit_id'];		
		$stmt_edit = $DB_con->prepare('SELECT bigdata, smalldata, picsource FROM firstaidcar WHERE id =:uid');
	...
	{
		header("Location: adminFirstAid.php");
	}
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE firstaidcar 
									     SET bigdata=:bigdata, 
										     smalldata=:smalldata, 
										     picsource=:upic 
								       WHERE id=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='adminFirstAid.php';
				</script>
                <?php
			}
			else{
